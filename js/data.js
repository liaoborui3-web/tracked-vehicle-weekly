...
window.WEEKLY_REPORTS = [
  ...
  },
  {
    week: 14,
    date: "6.1 - 6.7",
    title: "第14周项目周报：蓝车复现实车建图与 Nav2 路径规划",
    summary: "建图组本周因小黑车硬件问题转为使用蓝车实验，重新部署 ROS2 工作空间并验证前期工作的可复制性；完成 RoboSense、KISS-ICP、点云转 LaserScan、SLAM/Nav2 等环境配置，跑通 2D 栅格建图、地图保存、AMCL 定位和 RViz 目标点路径规划。",
    progress: 92,
    tags: ["建图定位", "蓝车", "RoboSense", "KISS-ICP", "SLAM", "Nav2", "底盘控制"],
    goals: [
      "在蓝车上重新部署并复现前期 RoboSense 建图与 Nav2 工作链路。",
      "完成雷达驱动、KISS-ICP、点云转激光扫描、SLAM/Nav2 等功能包编译和环境配置。",
      "验证 2D 栅格建图、地图保存、AMCL 定位和 RViz 目标点路径规划效果。",
      "联调 /cmd_vel、安全速度话题和底盘控制节点之间的控制链路。"
    ],
    tasks: [
      "针对小黑车硬件问题，将实验平台切换到蓝车，并验证前期工作的可复制性。",
      "在新车上重新部署 ROS2 工作空间，完成 RoboSense 雷达驱动、KISS-ICP、点云转激光扫描、SLAM/Nav2 等相关功能包的编译与环境配置。",
      "根据当前雷达型号修正雷达配置参数，解决点云解析报错问题。",
      "成功获取稳定的 /rslidar_points、/scan 和 /kiss/odometry 数据。",
      "完成基于激光雷达的 2D 栅格地图构建，并成功保存地图文件。",
      "在 Nav2 中加载保存的地图，通过 AMCL 完成定位，并在 RViz 中显示地图、实时雷达扫描、机器人位姿和全局规划路径。",
      "实现“在 RViz 中选定目标点后生成导航路径”的效果。",
      "对履带车底盘控制链路进行联调，确认 /cmd_vel、安全速度话题和底盘控制节点之间的连接关系。",
      "通过多次测试验证底盘可以响应 ROS2 速度指令。",
      "针对低速、小角速度和反向速度执行不稳定的问题，对 Nav2 速度参数、安全停止逻辑和控制流程进行初步调整。"
    ],
    issues: [
      "Nav2 能够规划路径并输出速度，但履带底盘在原地转向、低速跟踪和后退指令上表现不稳定。",
      "实际导航移动效果仍需继续调参和验证，问题主要集中在底盘执行层。",
      "虚拟机与小车之间偶尔出现 ROS2 discovery 不完整，需要通过重启 daemon 或重启相关节点恢复。",
      "需要进一步确认底盘控制协议对正反转和差速转向的支持情况。"
    ],
    nextPlan: [
      "继续优化 Nav2 控制参数，重点降低原地旋转需求并提高直行跟踪稳定性。",
      "进一步确认底盘控制协议对正反转和差速转向的支持情况。",
      "继续完善安全停止逻辑和控制流程，提升实车导航调试安全性。",
      "争取完成短距离自主导航闭环测试。"
    ],
    media: [
      {
        type: "image",
        src: "media/week14/rviz-2d-grid-map-overview.png",
        caption: "蓝车平台上基于 RoboSense 激光雷达构建的 2D 栅格地图，在 RViz 中显示地图整体轮廓和实时扫描区域。"
      },
      {
        type: "image",
        src: "media/week14/saved-2d-grid-map.png",
        caption: "保存后的 2D 栅格地图文件效果，用于后续在 Nav2 中加载并进行 AMCL 定位。"
      },
      {
        type: "image",
        src: "media/week14/nav2-global-path.png",
        caption: "在 RViz 中选定目标点后，Nav2 基于已加载地图生成全局规划路径。"
      },
      {
        type: "image",
        src: "media/week14/rviz-map-scan-nav2-debug.png",
        caption: "Nav2 联调界面中同时显示地图、里程计、路径、LaserScan 和 TF，用于排查定位与导航链路。"
      }
    ]
  }
];
